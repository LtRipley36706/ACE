...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        /// </summary>
        public bool FastTick => EnforceMovement || EnforceMovementSpeed || IsPKType;
        ...
        public bool UpdatePlayerPosition(ACE.Entity.Position newPosition, bool forceUpdate = false)
        {
            ...
            try
            {
                ...
                if (PhysicsObj != null)
                {
                    ...
                    if (EnforceMovementSpeed && success && !Teleporting)
                    {
                        ...
                        if (dist > PhysicsGlobals.EPSILON)
                        {
                            ...
                            {
                                if (MovementEnforcementCounter < PropertyManager.GetLong("enforce_player_movement_kick_threshold").Item || !PropertyManager.GetBool("enforce_player_movement_kick").Item)
                                {
                                ...
                                else
                                {
                                    ...
                                    Session.Terminate(SessionTerminationReason.MovementEnforcementThreshold, new GameMessageBootAccount(" because there is a divergence between your server and client locations"));
                                    log.Warn($"{Name} - INVALID MOVEMENT DETECTED - Speed: {dist.ToString("0.00")}/{currentMaxSpeed.ToString("0.00")} PrevMaxSpeed: {loggingPrevMaxMovementSpeed.ToString("0.00")}({loggingInertia}) FastTick: {FastTick} TimeSpam: {enforcementDeltaTime.ToString("0.00")} Velocity: {velocity.ToString("0.00")} actionsSinceLastMovementUpdate: {loggingHasPerformedActionsSinceLastMovementUpdate}");
                                    return false;
                                }
                            }
